...
# =============================================================================
# #### Agricultural data Uganda 2011-12
# =============================================================================

'''
DESCRIPTION:
    Uses the data from the 2011-2012 Integrated Survey in Agriculture from the UNPS (ISA-LSMS) to obtain:
        For the 2 main harvest of the year of the survey. (fall 2011 and spring 2012/)
    ...
    output: /data/data09/agric_data11.csv 
'''
import pandas as pd
import numpy as np
import os
...
#urban households are also asked for the agricultural questionnaire
...
ag3a = ag3a[["HHID", 'plotID', 'a3aq5', 'a3aq7',"a3aq8", 'a3aq15', 'a3aq17',"a3aq18",'a3aq24b', 'a3aq26',"a3aq27", 'a3aq39_workday', 'a3aq32', 'a3aq35a', 'a3aq35b', 'a3aq35c', 'a3aq36']]
...
ag4a.columns = ["HHID", 'plotID','cropID' , 'total_area', 'weightcrop', 'seed_cost']
...
ag5a.columns = ["HHID", 'plotID', "itemID", "total","unit", "tokg", "sell", "unit2", "value_sells", "trans_cost", "gift", "cons", "food_prod", "animal", "seeds", "stored"]
...
conversion_kg.to_csv('C:/Users/rodri/Dropbox/JMP/data/data11/kg_conversion_11a_2.csv', index=False)
# units were in text format. I manually changed them to the numeric using the 2011-12 codebook.
'''
### USE MEDIAN CONVERSION RATES ACROSS WAVES AND SEASONS
kg_units = pd.read_csv(folder2+'kg conversion/conversionkg_allwaves.csv')
kg_units = kg_units[['unit','cropID','kgconverter_11']]
...
ag5a.loc[(ag5a['unit']==99)|(ag5a['unit']==87)|(ag5a['unit']==80),'kgconverter_11'] = ag5a.loc[(ag5a['unit']==99)|(ag5a['unit']==87)|(ag5a['unit']==80),'tokg']

ag5a['kgconverter_11'] = ag5a['kgconverter_11'].fillna(ag5a['tokg'])

# Convert to kg
ag5a[["total_kg", "sell_kg", "gift_kg", "cons_kg", "food_prod_kg", "animal_kg", "seeds_kg", "stored_kg"]] = ag5a[["total", "sell", "gift", "cons", "food_prod", "animal", "seeds", "stored"]].multiply(ag5a["kgconverter_11"], axis="index")
...
cprices_nat = pd.read_csv(folder+"pricesfood11.csv")
cprices_reg = pd.read_csv(folder+"regionpricesfood11.csv")
cprices_county = pd.read_csv(folder+"countypricesfood11.csv")
cprices_district = pd.read_csv(folder+"districtpricesfood11.csv")

list_crops = ag5a['cropID'].unique()
...
ag3b = ag3b[["HHID", 'plotID', 'a3bq5', 'a3bq7', "a3bq8", 'a3bq15', 'a3bq17',"a3bq18",'a3bq24a','a3bq24b',"a3bq26", "a3bq27", 'a3ab39_workday', 'a3bq32', 'a3bq35a', 'a3bq35b', 'a3bq35c', 'a3bq36']]
...
ag3b = ag3b[["HHID", 'plotID', "org_fert", "chem_fert", 'pesticides','hhlabor', 'hired_labor', "a3bq36"]]
...
ag4b.columns = ["HHID", 'plotID','cropID' , 'total_area', 'weightcrop', 'seed_cost']
...
ag5b.columns = ["HHID", 'plotID', "cropID", "total","unit_name", "tokg", "sell", "unit2", "value_sells", "trans_cost", "gift", "cons", "food_prod", "animal", "seeds", "stored"]

unit_codes = pd.read_excel(folder2+'unit_codes_11b.xls')
ag5b = ag5b.merge(unit_codes, on='unit_name', how='left')

lele = ag5b['cropID'].value_counts()
ag5b['cropID'] = ag5b['cropID'].str.title()
ag5b.replace(['Avacoda' ], ['Avocado'], inplace=True)
...
conversion_kg.columns = ["unit",'cropID',"kgconverter"]
conversion_kg.to_csv('C:/Users/rodri/Dropbox/JMP/data/data11/kg_conversion_11b_2.csv', index=False)
'''
...
ag5b.loc[(ag5b['unit']==99)|(ag5b['unit']==87)|(ag5b['unit']==80),'kgconverter_11'] = ag5b.loc[(ag5b['unit']==99)|(ag5b['unit']==87)|(ag5b['unit']==80),'tokg']

ag5b['kgconverter_11'] = ag5b['kgconverter_11'].fillna(ag5b['tokg'])

# Convert to kg
ag5b[["total_kg", "sell_kg", "gift_kg", "cons_kg", "food_prod_kg", "animal_kg", "seeds_kg", "stored_kg"]] = ag5b[["total", "sell", "gift", "cons", "food_prod", "animal", "seeds", "stored"]].multiply(ag5b["kgconverter_11"], axis="index")
...
ag6a = ag6a[["HHID", "lvstid", "a6aq3a", "a6aq13a", "a6aq13b","a6aq14a","a6aq14b"]]
ag6a.columns = ['HHID',"lvstid","lvstk_big" ,"bought", "p_bought", "sold", "p_sold"]

#Obtain prices animals
prices = ag6a.groupby(by="lvstid")[["p_bought","p_sold"]].median()
p= prices.mean()
prices = prices.fillna(p)
prices.to_csv(folder+'prices_6a_2011.csv')
...
#Small animals=============================================================
ag6b = pd.read_stata(my_dirct+'AGSEC6B.dta')
ag6b = ag6b[["HHID", "lvstid", "a6bq3a", "a6bq13a","a6bq13b","a6bq14a","a6bq14b"]]
ag6b.columns = ['HHID', "lvstid", "lvstk_small", "bought", "p_bought", "sold", "p_sold"]

## Obtain prices --------------------------
prices = ag6b.groupby(by="lvstid")[["p_bought","p_sold"]].median()
p= prices.mean()
prices = prices.fillna(p)
prices.to_csv(folder+'prices_6b_2011.csv')
ag6b = pd.merge(ag6b, prices, on="lvstid", how='outer')

# Income selling small livestock-----------------------
ag6b["value_bought"] = ag6b.bought*ag6b.p_bought_y
ag6b["value_sold_small"] = 2*ag6b.sold*ag6b.p_sold_y
...
ag6c = ag6c[["HHID", "lvstid", "a6cq3a", "a6cq13a","a6cq13b","a6cq14a","a6cq14b"]]
ag6c.columns = ['HHID',"lvstid", "lvstk_poultry","bought", "p_bought", "sold", "p_sold"]

## Obtain prices --------------------------
prices = ag6c.groupby(by="lvstid")[["p_bought","p_sold"]].median()
p= prices.mean()
prices = prices.fillna(p)
prices.to_csv(folder+'prices_6c_2011.csv')
ag6c = pd.merge(ag6c, prices, on="lvstid", how='outer')

# Income selling poultry livestock-----------------------
##number sold in last 3 months
...
# Livestock inputs===========================================
ag7 = pd.read_stata(my_dirct+'AGSEC7B.dta')
...
ag7 = panel.sum(axis=0, level="HHID")
...
#Meat sold-----------------------------------------------------------
ag8a = pd.read_stata(my_dirct+'AGSEC8A.dta' )
ag8a = ag8a[["HHID", "a8aq5"]]
ag8a = ag8a.groupby(by='HHID').sum()
ag8a.reset_index(inplace=True)
ag8a.columns = ['HHID',"meat_sold"]

# Milk  Sold------------------------------------------------
ag8b = pd.read_stata(my_dirct+'AGSEC8B.dta' )
ag8b = ag8b[["HHID", "a8bq9"]]
ag8b.columns = ['HHID',  "milk_sold"]
ag8b = ag8b.groupby(by='HHID').sum()


# Eggs--------------------------------------------
ag8c = pd.read_stata(my_dirct+'AGSEC8C.dta' )
ag8c = ag8c[["HHID", "a8cq5"]]
ag8c.columns = ['HHID', "egg_sold"]
...
wealth.to_csv(folder+"wealth_agrls11.csv", index=False)
del data_hh['wealth_agrls'], data_hh['wealth_lvstk'], data_hh['farm_capital']
data_hh.to_csv(folder+"inc_agsec11.csv", index=False)
...
#### Control for inflation (non household-level dataset) =============================
basic = pd.read_stata(my_dirct+'GSEC1.dta', convert_categoricals=False )
basic = basic[["HHID","region","urban"]] 
basic['HHID'] = pd.to_numeric(basic['HHID'])
data = pd.merge(data, basic, on='HHID', how='left')

data['inflation_avg'] = 0.886005638
...
sumdatahh = data_check[['y','total_value_p_c_nat', 'total_value_p_c_reg', 'total_value_p_c_county', 'total_value_p_sell_nat','total_value_p_sell_reg','total_value_p_sell_county']].describe()

sum_m = data_check[['m', 'chem_fert','org_fert','pesticides', 'seed_cost','trans_cost','labor_payment']].describe(percentiles=percentiles)


outliers = data_check.loc[(data_check['y']>np.percentile(data_check['y'], 99)) | (data_check['A']>np.percentile(data_check['A'],100)),'HHID']
...
data.to_csv(folder+'agric_data11.csv', index=False)

print('data agric 2011 saved')
