...
"""
# =============================================================================
# #### Agricultural data Uganda 2015-16
# =============================================================================


'''
DESCRIPTION:
    Uses the data from the 2015-2016 Integrated Survey in Agriculture from the UNPS (ISA-LSMS) to obtain:
        For the 2 main harvest of the year of the survey. (fall 2016 and spring 2016/)
    ...
    output: inc_agsec15.csv (agric income and costs at hh level) wealth_agrls15.csv (agric wealth: livestock+farming capital), agric_data15.csv (outputs and inputs at crop-plot-hh level)
'''
import pandas as pd
import numpy as np
import os
...
basic.columns = ["HHID",'hh_2',"region","urban","year", "month","sregion",  'district_code', 'district', 'subcounty', 'parish']
...
ag3a = ag3a[["HHID", 'parcelID','plotID', 'a3aq5', 'a3aq7', "a3aq8", 'a3aq15', 'a3aq17',"a3aq18",'a3aq24a','a3aq24b', 'a3aq26',"a3aq27", 'a3aq33a_1', 'a3aq33b_1', 'a3aq33c_1', 'a3aq33d_1', 'a3aq33e_1', 'a3aq35a', 'a3aq35b','a3aq35c' ,"a3aq36"]]
...
ag3a['chem_fert'] = ag3a['p_chemfert']*ag3a['a3aq15']
ag3a['chem_fert'].describe()
ag3a['a3aq18'].describe()

##pesticides: small changes
ag3a['pesticides'] = ag3a['p_pest']*ag3a['a3aq24b']
ag3a['pesticides'].describe()
ag3a['a3aq27'].describe()
...
ag5a = ag5a[["HHID",'plotID',"cropID","a5aq6a","a5aq6c","a5aq6d","a5aq7a","a5aq7c","A5AQ7D","a5aq8","a5aq10","a5aq12","a5aq13","a5aq14a","a5aq114b","a5aq15","a5aq21"]]
...
conversion_kg.to_csv(folder2+'kg conversion/kg_conversion_15a.csv', index=False)
'''

### USE MEDIAN CONVERSION RATES ACROSS WAVES AND SEASONS
kg_units = pd.read_csv(folder2+'kg conversion/conversionkg_allwaves.csv')
kg_units = kg_units[['unit','cropID','kgconverter_15']]
...
ag5a.loc[(ag5a['unit']==99)|(ag5a['unit']==87)|(ag5a['unit']==80),'kgconverter_15'] = ag5a.loc[(ag5a['unit']==99)|(ag5a['unit']==87)|(ag5a['unit']==80),'tokg']

ag5a['kgconverter_15'] = ag5a['kgconverter_15'].fillna(ag5a['tokg'])


# Convert to kg
ag5a[["total_kg", "sell_kg", "gift_kg", "cons_kg", "food_prod_kg", "animal_kg", "seeds_kg", "stored_kg"]] = ag5a[["total", "sell", "gift", "cons", "food_prod", "animal", "seeds", "stored"]].multiply(ag5a["kgconverter_15"], axis="index")
...
### Use selling prices 2013 because 2015 ones don't make sense. Value selling is extremely small (problem inputing data I believe)
#Obtain Prices -------------------------------------------------------------
prices_nat = pd.read_csv(folder3+'nat_prices13a.csv')
prices_reg = pd.read_csv(folder3+'reg_prices13a.csv')
prices_county = pd.read_csv(folder3+'county_prices13a.csv')
prices_district = pd.read_csv(folder3+'district_prices13a.csv')
...
cprices_nat = pd.read_csv(folder+"pricesfood15.csv")
cprices_reg = pd.read_csv(folder+"regionpricesfood15.csv")
cprices_county = pd.read_csv(folder+"countypricesfood15.csv")
cprices_district = pd.read_csv(folder+"districtpricesfood15.csv")
...
for p in ['p_sell', 'p_c']:
    for area in ['_nat', '_reg', '_county','_district']:
        ...
        # Note that in this wave I cant use reported value sells, so total2_value is what it was total3.
        values_ag5a["total2_value_"+p+area] =  values_ag5a.loc[:,["sell_value_p_sell"+area,"gift_value_"+p+area,"cons_value_"+p+area,"food_prod_value_"+p+area,"animal_value_"+p+area, "stored_value_"+p+area]].sum(axis=1)
        values_ag5a["total2_value_"+p+area] = values_ag5a["total2_value_"+p+area].replace(0, np.nan)
        

check = (values_ag5a[['HHID','total_value_p_c_nat','total2_value_p_c_nat','total_value_p_c_reg','total2_value_p_c_reg']].groupby(by='HHID').sum()).replace(0,np.nan)/dollars
...
ag3b = ag3b[["HHID", 'parcelID','plotID', 'a3bq5', 'a3bq17','a3bq7','a3bq26', "a3bq8", 'a3bq15',"a3bq18",'a3bq24a','a3bq24b',"a3bq27", 'a3bq33a_1', 'a3bq33b_1', 'a3bq33c_1', 'a3bq33d_1', 'a3bq33e_1', 'a3bq35a', 'a3bq35b','a3bq35c' ,"a3bq36"]]
...
ag5b = ag5b[["HHID",'plotID',"cropID","a5bq6a","a5bq6c","a5bq6d","a5bq7a","a5bq7c","a5bq7d","a5bq8","a5bq10","a5bq12","a5bq13","a5bq14a","a5bq114b","a5bq15","a5bq21"]]
...
conversion_kg.columns = ["unit",'cropID',"kgconverter"]
conversion_kg.to_csv('folder2+'kg conversion/kg_conversion_15b.csv', index=False)
'''
...
ag5b['kgconverter_15'] = ag5b['kgconverter_15'].fillna(ag5b['tokg'])

# Convert to kg
ag5b[["total_kg", "sell_kg", "gift_kg", "cons_kg", "food_prod_kg", "animal_kg", "seeds_kg", "stored_kg"]] = ag5b[["total", "sell", "gift", "cons", "food_prod", "animal", "seeds", "stored"]].multiply(ag5b["kgconverter_15"], axis="index")
...
# For the second season some prices make sense but still many that do not. Very low values and many nans as well. Use 2013
prices_nat = pd.read_csv(folder3+'nat_prices13b.csv')
prices_reg = pd.read_csv(folder3+'reg_prices13b.csv')
prices_county = pd.read_csv(folder3+'county_prices13b.csv')
prices_district = pd.read_csv(folder3+'district_prices13b.csv')
...
for p in ['p_sell', 'p_c']:
    for area in ['_nat', '_reg', '_county', '_district']:
        ## Using each household reported value of sells plus using p_area to value non-sold production
        # Note that in this wave I cant use reported value sells, so total2_value is what it was total3.
        values_ag5b["total2_value_"+p+area] =  values_ag5b.loc[:,["sell_value_p_sell"+area,"gift_value_"+p+area,"cons_value_"+p+area,"food_prod_value_"+p+area,"animal_value_"+p+area, "stored_value_"+p+area]].sum(axis=1)
        values_ag5b["total2_value_"+p+area] = values_ag5b["total2_value_"+p+area].replace(0, np.nan)
        

check = (values_ag5b[['HHID','total_value_p_c_nat','total2_value_p_c_nat','total_value_p_c_reg','total2_value_p_c_reg']].groupby(by='HHID').sum()).replace(0,np.nan)/dollars
...
ag6a = ag6a[["hh", "LiveStockID", "a6aq3a", "a6aq13a", "a6aq13b","a6aq14a","a6aq14b"]]
ag6a.columns = ['hh',"LiveStockID","lvstk_big" ,"bought", "p_bought", "sold", "p_sold"]
...
prices.to_csv(folder+'prices_6a_2015.csv')
...
ag6a = ag6a.groupby(by='hh')[["value_sold_big", 'lvstk_big_value']].sum()
ag6a["hh"] = np.array(ag6a.index.values)


#Small animals=============================================================
ag6b = pd.read_csv(my_dirct+'agsec6b.csv', header=0, na_values='NA')
ag6b = ag6b[["hh", "ALiveStock_Small_ID", "a6bq3a","a6bq13a","a6bq13b","a6bq14a","a6bq14b"]]
ag6b.columns = ["hh", "ALiveStock_Small_ID", "lvstk_small",  "bought", "p_bought", "sold", "p_sold"]

## Obtain prices --------------------------
prices = ag6b.groupby(by="ALiveStock_Small_ID")[["p_bought","p_sold"]].median()
p= prices.mean()
prices = prices.fillna(p)
prices["ALiveStock_Small_ID"] = np.array(prices.index.values)
prices.to_csv(folder+'prices_6b_2015.csv')
prices.index.name = None
...
ag6b = ag6b.groupby(by='hh')[["value_sold_small", 'lvstk_small_value']].sum()
ag6b["hh"] = np.array(ag6b.index.values)


#Poultry animals==========================================
ag6c = pd.read_csv(my_dirct+'agsec6c.csv', header=0, na_values='NA')
ag6c = ag6c[["hh", "APCode", "a6cq3a", "a6cq13a","a6cq13b","a6cq14a","a6cq14b"]]
ag6c.columns = ['hh',"APCode", "lvstk_poultry" ,"bought", "p_bought", "sold", "p_sold"]

## Obtain prices --------------------------
prices = ag6c.groupby(by="APCode")[["p_bought","p_sold"]].median()
p= prices.mean()
prices = prices.fillna(p)
prices["APCode"] = np.array(prices.index.values)
prices.to_csv(folder+'prices_6c_2015.csv')
prices.index.name = None
...
ag6c = ag6c.groupby(by='hh')[["value_sold_poultry", 'lvstk_poultry_value']].sum()
ag6c["hh"] = np.array(ag6c.index.values)


# Livestock inputs===========================================
ag7 = pd.read_csv(my_dirct+'agsec7b.csv', header=0, na_values='NA')
ag7 = ag7[["hh", "AGroup_ID","a7bq2e", "a7bq3f", "a7bq5d","a7bq6c", "a7bq7c", "a7bq8c"]]
values = ["a7bq2e", "a7bq3f", "a7bq5d", "a7bq6c","a7bq7c", "a7bq8c"]
index = ["hh", "AGroup_ID"]
...
#Merge datasets------------------------------------------------------
ag6a.index.name = None
ag6b.index.name = None
ag6c.index.name = None
ag7.index.name = None
ag8a.index.name = None
#ag8c.index.name = None
...
livestock.replace(0,np.nan,inplace=True)
...
animal_c = pd.read_csv(folder+"c_animal15.csv")
...
data_hh[['org_fert', 'chem_fert', 'pesticides', 'labor_payment',  'seed_cost', 'trans_cost']] = remove_outliers(data_hh[['org_fert', 'chem_fert', 'pesticides', 'labor_payment',  'seed_cost', 'trans_cost']],hq=0.99)
...
sumdata_hh = data_hh[['total2_value_p_c_nat','total2_value_p_c_reg', 'total2_value_p_c_county', 
                       'total2_value_p_c_nat','total2_value_p_sell_nat','total2_value_p_sell_reg', 
                       'total2_value_p_sell_county',  'total2_value_p_sell_nat']].describe()/dollars
...
wealth = data_hh[['HHID',"hh","wealth_agrls","wealth_lvstk",'farm_capital']]

del data_hh['HHID']
### Agricultural wealth and income to compute total household W and I
wealth.to_csv(folder+"wealth_agrls15.csv", index=False)
del data_hh['wealth_agrls'], data_hh['wealth_lvstk'], data_hh['farm_capital']
data_hh.to_csv(folder+"inc_agsec15.csv", index=False)
...
data['inflation_avg'] = 1.099176036

## Control for inflation and convert to US 2013 $: .div(data.inflation, axis=0)
...
priceslist = [ "p_c_nat", "p_c_reg", "p_c_county", "p_c_district"] 
for p in priceslist:
    data[['total_value_'+p, 'total2_value_'+p, "sell_value_"+p, "cons_value_"+p, "stored_value_"+p,"gift_value_"+p]] = data[['total_value_'+p, 'total2_value_'+p, "sell_value_"+p, "cons_value_"+p, "stored_value_"+p, "gift_value_"+p]].div(data.inflation_avg, axis=0)/dollars

pricelist_sell = ["p_sell_nat","p_sell_reg","p_sell_county","p_sell_district"]
for p in pricelist_sell:
    data[['total_value_'+p, 'total2_value_'+p, "sell_value_"+p, "cons_value_"+p, "stored_value_"+p,"gift_value_"+p]] = data[['total_value_'+p, 'total2_value_'+p,"sell_value_"+p, "cons_value_"+p, "stored_value_"+p,"gift_value_"+p]].div(data.inflation_avg, axis=0)/dollars
...
sumdatahh = data_check[['y','total_value_p_c_nat', 'total_value_p_c_reg', 'total_value_p_c_county', 'total_value_p_sell_nat','total_value_p_sell_reg','total_value_p_sell_county',
                     'total2_value_p_c_nat', 'total2_value_p_c_reg', 'total2_value_p_c_county', 'total2_value_p_sell_nat','total2_value_p_sell_reg','total2_value_p_sell_county']].describe(percentiles=percentiles)

data['cropID'].replace(np.nan, 'nan', inplace=True)
data['cropID'] = data['cropID'].astype(str)

outliers = data_check.loc[(data_check['y']>np.percentile(data_check['y'],99)) | (data_check['A']>np.percentile(data_check['A'],100)) | (data_check['m']>np.percentile(data_check['m'], 100)),'HHID']
...
sumdatahh2 = data_check[['y','m','A','l', 'total_value_p_c_reg', 'total_value_p_c_county', 'total2_value_p_c_reg', 'total2_value_p_c_county', 'total2_value_p_c_district']].describe(percentiles=percentiles)



# save dataset =======================================
data.to_csv(folder+'agric_data15.csv', index=False)
print('data agric 15 saved')
